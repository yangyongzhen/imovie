<template>
	<view @click.native="previewImage" class="trigger-content" :style="{
		...
		<!-- 加载中 -->
		<image :src="loadSrc" class="trigger-image muqain-load" @load="init" mode="scaleToFill" :style="{
				opacity:isShow?'0':'1',
				borderRadius,
				width,
				height,
		...
		<!-- 加载成功 -->
		<image class="trigger-image" @load="load" @error="error" v-if="status==1" :src="src||'-'" :mode="imageModel"
			:style="{
				opacity:isShow?'1':'0',
				borderRadius,
				width,
		...
		<!-- 加载失败 -->
		<image class="trigger-image" v-if="status==2" :src="errorSrc" mode="scaleToFill" :style="{
			opacity:isShow?'1':'0',
			borderRadius,
			width,
			height,
